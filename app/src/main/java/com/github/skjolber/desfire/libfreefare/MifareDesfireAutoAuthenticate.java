...
public class MifareDesfireAutoAuthenticate {
	...
	public static void mifare_desfire_auto_authenticate (MifareTag tag, byte key_no) throws Exception
	{
	    ...
	    case 0x00:
		key = MifareDesfireKey1.mifare_desfire_des_key_new_with_version (key_data_null);
		break;
	    case 0x42:
		key = MifareDesfireKey1.mifare_desfire_aes_key_new_with_version (key_data_aes, key_data_aes_version);
		break;
	    case (byte) 0xAA:
	    	Log.d(TAG, "Authenticate using DES key");
		key = MifareDesfireKey1.mifare_desfire_des_key_new_with_version (key_data_des);
		break;
	    case (byte) 0xC7:
		key = MifareDesfireKey1.mifare_desfire_3des_key_new_with_version (key_data_3des);
		break;
	    case 0x55:
		key = MifareDesfireKey1.mifare_desfire_3k3des_key_new_with_version (key_data_3k3des);
		break;
	    default:
